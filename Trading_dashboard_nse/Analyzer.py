...
class NiftyTrendAnalyzer:
    ...
    def calculate_portfolio_greeks(self) -> Dict:
        """
        Calculate net portfolio Greeks across all strikes
        Useful for validating bullish/bearish bias
        """
        greeks_data = self.calculate_option_greeks()
        
        if not greeks_data.get('greeks'):
            return {}
        
        # Sum deltas and gammas
        total_ce_delta = 0
        total_pe_delta = 0
        total_ce_gamma = 0
        total_pe_gamma = 0
        ...
        for option in greeks_data['greeks']:
            # Weight by OI for more accurate portfolio view
            ce_weight = option['ce']['oi'] if option['ce']['oi'] > 0 else 0
            pe_weight = option['pe']['oi'] if option['pe']['oi'] > 0 else 0
            
            total_ce_delta += option['ce']['delta'] * ce_weight
            total_pe_delta += option['pe']['delta'] * pe_weight
            total_ce_gamma += option['ce']['gamma'] * ce_weight
            total_pe_gamma += option['pe']['gamma'] * pe_weight
            total_ce_theta += option['ce']['theta'] * ce_weight
            total_pe_theta += option['pe']['theta'] * pe_weight
            total_ce_vega += option['ce']['vega'] * ce_weight
            total_pe_vega += option['pe']['vega'] * pe_weight
        
        # Net delta (CE delta = bullish, PE delta = bearish)
        net_delta = total_ce_delta - total_pe_delta
        
        # Interpretation
        if net_delta > 1000:
            delta_signal = "STRONG BULLISH"
            delta_bias = f"Large net positive delta: {round(net_delta, 0)}"
        elif net_delta > 300:
            delta_signal = "BULLISH"
            delta_bias = f"Positive net delta: {round(net_delta, 0)}"
        elif net_delta < -1000:
            delta_signal = "STRONG BEARISH"
            delta_bias = f"Large net negative delta: {round(net_delta, 0)}"
        elif net_delta < -300:
            delta_signal = "BEARISH"
            delta_bias = f"Negative net delta: {round(net_delta, 0)}"
        else:
            delta_signal = "NEUTRAL"
            delta_bias = f"Balanced delta: {round(net_delta, 0)}"
        
        # Gamma risk (high gamma = higher risk of rapid reversals)
        total_gamma = total_ce_gamma + total_pe_gamma
        gamma_signal = "LOW" if total_gamma < 5 else "MODERATE" if total_gamma < 20 else "HIGH"
        
        return {
            'portfolio_delta': round(net_delta, 2),
            'ce_delta': round(total_ce_delta, 2),
            'pe_delta': round(total_pe_delta, 2),
            'delta_signal': delta_signal,
            'delta_bias': delta_bias,
            'portfolio_gamma': round(total_gamma, 4),
            'gamma_risk': gamma_signal,
            'portfolio_theta': round(total_ce_theta + total_pe_theta, 2),
            'portfolio_vega': round(total_ce_vega + total_pe_vega, 2),
            'greeks_data': greeks_data['greeks']
        }
    
    def identify_fast_moving_options(self) -> Dict:
        """
        Identify fast-moving options (high Gamma) that could be trading opportunities
        Gamma indicates how much delta changes with price movement
        """
        portfolio_greeks = self.calculate_portfolio_greeks()
        
        if not portfolio_greeks.get('greeks_data'):
            return {}
        
        fast_movers = []
        
        for option in portfolio_greeks['greeks_data']:
            strike = option['strike']
            
            # Identify options with high gamma (fast moving)
            ce_gamma = option['ce']['gamma']
            pe_gamma = option['pe']['gamma']
            
            # Higher OI + Higher Gamma = Fast moving option
            ce_momentum = ce_gamma * (option['ce']['oi'] / 1000) if option['ce']['oi'] > 0 else 0
            pe_momentum = pe_gamma * (option['pe']['oi'] / 1000) if option['pe']['oi'] > 0 else 0
            
            # Determine direction based on Delta signed by OI
            ce_bullish_score = option['ce']['delta'] * (option['ce']['oi'] / 1000000) if option['ce']['oi'] > 10000 else 0
            pe_bullish_score = abs(option['pe']['delta']) * (option['pe']['oi'] / 1000000) if option['pe']['oi'] > 10000 else 0
            
            if ce_momentum > 0.01:
                direction = "CALLS" if ce_bullish_score > 0 else "PUTS"
                fast_movers.append({
                    'strike': strike,
                    'type': 'CE',
                    'momentum': round(ce_momentum, 4),
                    'gamma': round(ce_gamma, 6),
                    'delta': round(option['ce']['delta'], 4),
                    'oi': option['ce']['oi'],
                    'iv': option['ce']['iv'],
                    'direction': 'BULLISH' if option['ce']['delta'] > 0.5 else 'BEARISH' if option['ce']['delta'] < 0.2 else 'NEUTRAL',
                    'velocity': 'FAST' if ce_momentum > 0.05 else 'MODERATE'
                })
            
            if pe_momentum > 0.01:
                direction = "PUTS" if pe_bullish_score > 0 else "CALLS"
                fast_movers.append({
                    'strike': strike,
                    'type': 'PE',
                    'momentum': round(pe_momentum, 4),
                    'gamma': round(pe_gamma, 6),
                    'delta': round(option['pe']['delta'], 4),
                    'oi': option['pe']['oi'],
                    'iv': option['pe']['iv'],
                    'direction': 'BULLISH' if option['pe']['delta'] > -0.5 else 'BEARISH' if option['pe']['delta'] < -0.8 else 'NEUTRAL',
                    'velocity': 'FAST' if pe_momentum > 0.05 else 'MODERATE'
                })
        
        # Sort by momentum
        fast_movers.sort(key=lambda x: x['momentum'], reverse=True)
        
        # Get top 5
        top_movers = fast_movers[:5]
        
        # Count by direction
        bullish_count = sum([1 for m in top_movers if m['direction'] == 'BULLISH'])
        bearish_count = sum([1 for m in top_movers if m['direction'] == 'BEARISH'])
        
        overall_direction = "BULLISH" if bullish_count > bearish_count else "BEARISH" if bearish_count > bullish_count else "MIXED"
        ...
        return {
            ...
            'total_identified': len(fast_movers)
        }
    ...
    def generate_composite_signal(self) -> Dict:
        ...
        portfolio_greeks = self.calculate_portfolio_greeks()
        fast_movers = self.identify_fast_moving_options()
        ...
        return {
            ...
            'fast_moving_options': fast_movers,
            'individual_signals': {
                'PCR': pcr['signal'],
                'Max Pain': max_pain['signal'],
                'OI Buildup': oi_buildup['signal'],
                'Buy/Sell Pressure': pressure['signal'],
                'Delta': portfolio_greeks.get('delta_signal', 'N/A')
            },
            'spot': spot,
            'rows': options,
            'total_call_oi': total_call_oi,
