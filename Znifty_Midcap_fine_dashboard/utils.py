...
def fetch_midcap_option_chain(exp_date, symbol = "MIDCPNIFTY"):
    print(f"[DEBUG] fetch_midcap_option_chain() called: exp_date={exp_date}, symbol={symbol}")
...
def write_to_local(data_to_write ):
    ...
    filename = "data/MIDCAP_processed"+str(timestamp_str)+".json"
    try:
...
def derive_midcap_option_chain(chain):
...
# ...existing code...
def derive_and_classify_midcap_option_chain(chain):
    records = chain["records"]
    rows = records["data"]
    spot_raw = records.get("underlyingValue")
    spot = float(str(spot_raw).replace(",", ""))
