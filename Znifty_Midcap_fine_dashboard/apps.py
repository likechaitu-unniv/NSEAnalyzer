"""
Unified Flask Web Application for Nifty Midcap Trend Analysis
Combines both stock analyzer and option chain analyzer
"""

from flask import Flask, render_template, jsonify, request
from flask_socketio import SocketIO, emit
from Analyzer import MidcapTrendAnalyzer, fetch_midcap_option_chain
...
def update_expiry_if_needed():
    """Update expiry to nearest date if current expiry >= nearest date"""
    global expiry
    if expiryDates:
        nearest_date = expiryDates[0]
        # Parse dates to compare
        try:
            from datetime import datetime
            current_expiry_date = datetime.strptime(expiry, '%d-%b-%Y')
            nearest_expiry_date = datetime.strptime(nearest_date, '%d-%b-%Y')
            
            if current_expiry_date >= nearest_expiry_date:
                expiry = nearest_date
                print(f"[DEBUG] Expiry updated to: {expiry}")
        except Exception as e:
            print(f"[ERROR] Error comparing expiry dates: {e}")

# Initialize trend storage with current expiry
trend_storage = TrendStorage(expiry=expiry)


class MidcapStockAnalyzer:
    """Business logic for analyzing Nifty Midcap 50 stock data"""
    
    def __init__(self):
        print("[DEBUG] Initializing MidcapStockAnalyzer")
        self.api_url = "https://www.nseindia.com/api/equity-stockIndices?index=NIFTY%20MIDCAP%20SELECT"
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
            'Accept': 'application/json',
            'Accept-Language': 'en-US,en;q=0.9',
        }
        print("[DEBUG] MidcapStockAnalyzer initialized")
    
    def fetch_midcap_data(self):
        """Fetch data from NSE API"""
        print("[DEBUG] fetch_midcap_data() called")
        try:
            ...
            data = response.json()
            print(f"[DEBUG] Response data received")
            if 'data' in data:
                print(f"[DEBUG] Total stocks received: {len(data['data'])}")
                print(f"[DEBUG] Stocks data: {data['data'][:5]}")  # Log first 5 stocks
            return data
        except Exception as e:
    ...
    def analyze(self):
        ...
        raw_data = self.fetch_midcap_data()
        ...
        stocks = [s for s in raw_data['data'] 
                 ...
                 and s.get('symbol') not in ['MIDCPNIFTY', 'Nifty Midcap 50', 'Midcap 50']]
...
stock_analyzer = MidcapStockAnalyzer()
...
def transmit_option_analysis():
    ...
    while True:
        try:
            # Update expiry if needed based on nearest date
            update_expiry_if_needed()
